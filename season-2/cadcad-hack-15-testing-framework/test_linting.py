...
def test_lint_state_variables():
    ...
    state_update_blocks_state_variables = set(
        [
            ...
            for state_variable in block.get('variables')
        ]
    )
...
def test_lint_parameters():
    ...
    model_modules = inspect.getmembers(parts, inspect.ismodule)
    ...
    print([key for key in system_params.keys() if key not in consts])
    # Assert all System Parameter keys are in consts
    assert all([key in consts for key in system_params.keys()])
